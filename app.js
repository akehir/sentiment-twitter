...
function startApp() {
    ...
    liveModeIntervalId = setInterval(function () {
        checkNewKeywords();
    }, 2000);
    loadDemoData();
...
function pushData() {
    ...
    if (pushLine == null)
        pushLine = 1000;
    debugLog += " push start";
    fakeDataPushId = setInterval(function () {
        fakeDataPush()
    }, 1000);
...
app.get('/addOneMode', function (req, res) {
    ...
    if (addOneMode == false) {
        ...
        addSingleTweetIntervalId = setInterval(function () {
            monitorSingleWords();
        }, 2000);
        //establishTwitterConnection();
...
function establishTwitterConnection() {
    ...
    debugLog += ' establish';
    var monitoringPhrase = monitoringKeywords.map(function (elem) {
        return elem.phrase;
    }).join(",");
